...
const MasterSampleEditModal: React.FC<EditModalProps> = ({
  ...
  const handleGeneratePDF = async () => {
    const typeObj = types.find(t => t.id === masterType);
    const processObj = processes.find(p => p.id === processName);
    
    const preparedBy = fullData?.created_by 
      ? `${fullData.created_by.first_name} ${fullData.created_by.last_name}` 
      : '';
      
    const dateCreated = fullData?.date_created 
      ? new Date(fullData.date_created).toLocaleDateString("pl-PL") 
      : '';

    const labelData: LabelData = {
      masterTypeName: typeObj?.name || '',
      processName: processObj?.name || '',
      projectName: projectName,
      endcodes: endcodes,
      codeSmd: codeSmd,
      sn: sn,
      pcbRevCode: pcbRevCode,
      preparedBy: preparedBy,
      dateCreated: dateCreated,
      expireDate: new Date(expireDate).toLocaleDateString("pl-PL") // Formatowanie daty
    };

    try {
      const blob = await pdf(<LabelPDF data={labelData} />).toBlob();
      const url = URL.createObjectURL(blob);
      window.open(url, '_blank');
    } catch (err) {
      console.error("Błąd generowania PDF:", err);
      setError("Nie udało się wygenerować etykiety.");
    }
  };
        ...
        <form onSubmit={handleSubmit} className="g-modal-form">
          {loading || loadingDicts ? (
            ...
            <>
              ...
              <div className="g-modal-actions" style={{ justifyContent: 'space-between', display: 'flex', gap: '10px', width: '100%' }}>
